...
class WebSocketSink(GameEventSink):
    ...
    def on_public_action(self, speaker, message: str, color: str = "", animate: bool = True, should_hold: bool = True, directed_to_name=None, is_reply: bool = False):
        if directed_to_name:
            message = f"@{directed_to_name} - {message}"
        speaker_name = speaker.name if hasattr(speaker, "name") else str(speaker)
        event = {"type": "public_action", "speaker": speaker_name, "message": message, "animate": animate, "should_hold": should_hold}
        if is_reply:
            event["child"] = True
        self._send(event)
